...
1. [Google AI Studio](https://aistudio.google.com/app/apikey) 에서 API 키 발급 (무료 등급 가능)
2. `GEMINI_API_KEY` 에 입력. 모델은 기본 `gemini-2.5-flash`.
...
3. **Environment Variables** 에 4개 등록:
...
> `GEMINI_API_KEY` 는 `NEXT_PUBLIC_` 접두사가 없어 서버에서만 쓰이며 노출되지 않습니다.
...
app/
  ...
  api/generate/route.js  서버 측 AI 호출(Gemini, 키 비노출)
lib/
...
- AI 제공자를 Gemini 대신 다른 것으로 바꾸려면 `app/api/generate/route.js` 의 `callGemini` 만 교체하세요.
